...
$statement = $pdo->prepare("SELECT * FROM apontamento where id = :id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' ){
    ...
    if(isset($_FILES['image_file'])) {
      ...
      if(empty($erros)){
        ...
        if($img_ready) {
            $statement = $pdo->prepare("UPDATE apontamento SET id_email = :id_email, id_tipoApontamento = :id_tipoApontamento, descricao = :descricao, 
            informacao = :informacao, data_criacao = :data_criacao, image_path = :image_path WHERE id = :id");
        } else {
            $statement = $pdo->prepare("UPDATE apontamento SET id_email = :id_email, id_tipoApontamento = :id_tipoApontamento, descricao = :descricao, 
                ...
                <div class="apontamentosformscard-body">
                    <div class="content" style="margin-bottom: 5%">
                            ...
                            <?php if(!empty($tipos)){ ?>
                                <select style="width: 80%" name="tipo">
                                    <?php foreach($tipos as $tipo) { ?>
                                        <option value="<?php echo $tipo['id_tipoApontamento'] ?>" <?php if($apontamento['id_tipoApontamento'] == $tipo['id_tipoApontamento']) echo 'selected' ?>><?php echo $tipo['descricao']?></option>
                                    <?php } ?>
                                </select>
                            <?php } else
                        ...
                        <div>
                            ...
                            <br>
                            <input style="width: 80%" type="text" name="descricao" id="txt" placeholder="Descrição" value="<?php echo $apontamento['descricao'] ?>" required>
                        </div>
                        <div>
                            <label for="">Informação</label>
                            <br>
                            <input style="width: 80%" type="text" name="informacao" id="txt" placeholder="Informação" value="<?php echo $apontamento['informacao'] ?>" required>
                        </div>
                        <div>
                            <label for="">Data</label>
                            <br>
                            <input style="width: 80%" type="date" name="data" id="txt" value="<?php if(empty($dataCriacao)) echo (date('Y-m-d')); else echo($apontamento['data_criacao']);  ?>" required>
                        </div>
                        <div>
